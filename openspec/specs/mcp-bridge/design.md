...
| `get_events` | `device?`, `timeout_s?` | Structured sibling of `get_messages`: returns a JSON array, one object per event, tagged `message` / `ui_event` / `file` / `connected` / `disconnected` with its source `device` — the same vocabulary `azula watch --json` streams. A `ui_event` carries the A2UI payload verbatim and a `file` its attachment facts (plus `caption` when one was sent), neither of which survive the one-line rendering. With `timeout_s` it waits for the inbox to become non-empty before draining and returns an empty list if the timeout elapses; without, it drains whatever is pending immediately. Shares one queue with `get_messages`/`wait_for_reply` | n/a (reads only) |
...
| `set_typing` | `device`, `on` | Sends a bare `thinking(on)` frame — the conversation's activity indicator, with no message text. `send_message` brackets its own text with this, so before this tool there was no way to show a turn was working before it had anything to say | **Live only.** Errors immediately if the device is unreachable; never queued to the relay or the local mailbox, since an indicator replayed later claims activity that has already ended |
