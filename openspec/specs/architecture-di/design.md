...
- **terminal** — `terminal-api` (the `TerminalEmulator` engine, `PredictionEngine`,
  ...
  [`terminal.md`](../terminal/design.md).
...
stream outright ([`invitations.md`](../invitations/design.md)). And its id must differ from
...
- **`MediaService`** — the media-attachment lifecycle: blob-backed send/offer,
  ...
  [`media-transfer.md`](../media-transfer/design.md).
