...
**iOS release prerequisite:** the bundle ids are the real `app.azula` +
`app.azula.AzulaShare`, `DEVELOPMENT_TEAM` is the real team id (`EB8N37743E`),
export compliance is declared, and the pipeline is wired (see [release.md](release.md)).
What remains is Apple-side account setup that can't be done from the repo: App IDs
for both bundle ids with the App Groups capability, `group.app.azula` registered
and assigned to both, the App Store Connect app record + API key, and the four
Apple secrets on the repo. Simulator builds use ad-hoc signing and need none of it.
